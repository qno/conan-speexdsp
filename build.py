from bincrafters import build_template_default
import os

# see https://github.com/bincrafters/bincrafters-package-tools
os.environ["BINTRAY_REPOSITORY"]            = "conan-public"
os.environ["CONAN_UPLOAD"]                  = "https://api.bintray.com/conan/qno/conan-public"
os.environ["CONAN_USERNAME"]                = "qno"
os.environ["CONAN_PASSWORD"]                = os.environ["BINTRAY_API_KEY"]
os.environ["CONAN_LOGIN_USERNAME"]          = os.environ["BINTRAY_LOGIN"]
os.environ["CONAN_STABLE_BRANCH_PATTERN"]   = "stable/*"
os.environ["CONAN_UPLOAD_ONLY_WHEN_STABLE"] = False
os.environ["CONAN_DOCKER_32_IMAGES"]        = True


def _is_static_msvc_build(build):
  if build.options["SpeexDSP:shared"] == False and build.settings["compiler"] == "Visual Studio":
    return False
  else:
    return True

if __name__ == "__main__":
  builder = build_template_default.get_builder()
  builder.builds = filter(_is_static_msvc_build , builder.items)
  builder.run()
